...
const MenuDetailModal = ({ menu, visible, onClose, userXu, onPurchaseMenu }) => {  
    ...
    const [menuStatus, setMenuStatus] = useState({ isUnlocked: false }); // Trạng thái khởi tạo  
    ...
    const handleUnlock = async () => {  
        ...
        Modal.confirm({  
            ...
            onOk: async () => {  
                setLoading(true);  
                try {  
                    const result = await onPurchaseMenu(menu._id); // Gọi hàm mua menu  
                    if (result.success) {  
                        setMenuStatus({ isUnlocked: true }); // Cập nhật trạng thái menu  
                        message.success(`Menu unlocked! Remaining xu: ${result.Xu}`); // Thông báo thành công  
                    } else {  
                        message.error(result.message || 'Unable to unlock the menu');  
                    }  
                } catch (error) {  
    ...
    const renderMenuContent = () => {  
        if (!menuStatus) return <Skeleton active />; // Hiển thị loading nếu chưa có status  
        ...
        return menuStatus.isUnlocked || menu.defaultStatus === 'unlock' ? (  
            <div>  
                ...
                <div className="mt-4">  
                    ...
                    {/* Hiện các thông tin khác nếu cần */}  
                </div>  
            </div>  
        ) : (  
            <div>  
