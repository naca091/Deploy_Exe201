...
const MenuDetailModal = ({  
  ...
  const handleUnlock = () => {  
    ...
    if (userXu >= menu.unlockPrice) {  
      Modal.confirm({  
        ...
        onOk: async () => {  
          try {  
            ...
            console.log('Updated user xu after purchase:', response.userXu); // In ra số xu mới cập nhật  
          } catch (error) {  
      ...
      message.warning("You don't have enough xu to unlock this menu."); // Thông báo nếu số xu không đủ  
    }  
  };  
