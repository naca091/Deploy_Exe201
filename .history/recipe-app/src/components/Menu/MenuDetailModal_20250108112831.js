...
const MenuDetailModal = ({ menu, visible, onClose, userCoins = 0, onPurchaseMenu }) => {  
  // Trong component sử dụng MenuDetailModal  
const fetchUserData = async () => {  
  try {  
    const response = await axios.get(`/api/users/${currentUserId}`); // Đảm bảo URL đúng  
    setCurrentUser(response.data); // Giả sử setCurrentUser là hàm cập nhật trạng thái người dùng  
  } catch (err) {  
  ...
  const handleUnlock = () => {  
    console.log("User Coints before unlock attempt:", userCoins);  
