...
const MenuDetailModal = ({   
  ...
  const handleUnlock = () => {  
    ...
    if (userCoins >= menu.unlockPrice) {  
      Modal.confirm({  
        ...
        onOk: async () => {  
          try {  
            ...
            console.log("Updated user coints after purchase:", response.data.userCoins); // In ra số coins mới cập nhật  
          } catch (error) {  
      ...
      message.warning("You don't have enough coints to unlock this menu.");  
    }  
  };  
