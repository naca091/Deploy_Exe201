...
// Route mua menu  
router.post('api//menus/purchase', async (req, res) => {
  console.log('Request received:', req.body);
  const { menuName, userEmail } = req.body;

  try {
    const menu = await Menu.findByName(menuName);
    if (!menu) return res.status(404).json({ success: false, message: 'Menu not found' });

    if (!menu.purchasedBy.includes(userEmail)) {
      menu.purchasedBy.push(userEmail); // Thêm email vào purchasedBy
      await menu.save();
    }

    res.json({ success: true, updatedMenu: menu });
  } catch (error) {
    console.error('Purchase menu error:', error);
    res.status(500).json({ success: false, message: 'Failed to purchase menu' });
  }
});
