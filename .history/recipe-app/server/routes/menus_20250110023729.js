...
// Route mua menu  
// Purchase menu route  
router.post('/purchase/:menuId', async (req, res) => {  
  const menuId = req.params.menuId; // This is the menuId from your schema  
  const userId = req.user.id; // Assuming you have user authentication middleware  

  try {  
      // Find the menu by menuId (not the MongoDB _id)  
      ...
      if (user.xu < menu.unlockPrice) {  
          return res.status(400).json({ success: false, message: 'Insufficient xu to purchase the menu.' });  
      }  

      // Deduct xu and add user ID to purchasedBy  
      ...
      res.json({ success: true, message: 'Menu unlocked successfully!' });  
  } catch (error) {  
