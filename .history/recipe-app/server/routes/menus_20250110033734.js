...
// Route mua menu  
router.post('/api/menus/purchase/:id', authMiddleware, async (req, res) => {  
  const menuId = req.params.menuId; // Lấy _id trực tiếp từ params  
  const userId = req.user._id; // Lấy userId từ req.user  

  console.log('Received menuId:', menuId); // Log menuId  
  console.log('User from req:', req.user); // Log thông tin user   

  try {  
      const menu = await Menu.findById(menuId); // Tìm menu theo _id  
  ...
  if (menu) {  
    // Nếu bạn muốn cập nhật purchaseBy  
    menu.purchaseBy = userId; // cập nhật vào đây  
    // Hoặc, không cập nhật but vẫn cho phép người dùng mua  
    await menu.save();  
    res.json({  
        success: true,  
        message: 'Purchase successful!',  
        menu: menu // trả lại menu đã cập nhật hoặc không  
    });  
} else {  
    res.status(404).json({  
        success: false,  
