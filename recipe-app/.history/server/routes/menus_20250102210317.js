...
// Create menu
router.post('/api/menus', auth, checkRole(['admin']), upload.single('image'), async (req, res) => {
    try {
        const menuData = {
            ...req.body,
            image: req.file.path,
...
// Update menu
router.put('/api/menus/:id', auth, checkRole(['admin']), upload.single('image'), async (req, res) => {
    try {
...
router.delete('/api/menus/:id', auth, checkRole(['admin']), async (req, res) => {
    try {
        const menu = await Menu.findByIdAndUpdate(
            req.params.id,
            { isActive: false },
