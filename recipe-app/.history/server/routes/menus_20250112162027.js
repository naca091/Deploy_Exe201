...
router.post("/api/menus/:menuId/purchase", auth, async (req, res) => {
  ...
  try {
    ...
    console.log('Purchase attempt for menuId:', menuId); // Debug log
    ...
    console.log('Current user xu:', user.xu); // Debug log

    const menu = await Menu.findById(menuId).session(session);
    if (!menu) {
      throw new Error('Menu not found');
    }
    console.log('Menu unlock price:', menu.unlockPrice); // Debug log
