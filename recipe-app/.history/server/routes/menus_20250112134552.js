...
// Route mua menu  
router.post('/api/menus/:menuId/purchase', async (req, res) => {
  ...
  try {
    ...
    const alreadyPurchased = user.purchasedMenus.some(p => p.menuId.equals(menuId));
    if (alreadyPurchased) {
    ...
    if (user.xu < menu.unlockPrice) {
      throw new Error('Insufficient xu balance');
    }

    // Trừ xu và thêm menu vào danh sách đã mua
