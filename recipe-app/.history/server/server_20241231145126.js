...
const initializeRoles = require('./initDb');
...
app.post('/api/users/register', async (req, res) => {
    try {
        ...
        // Find the member role (roleId: 1)
        const memberRole = await Role.findOne({ roleId: 1 });
        if (!memberRole) {
            return res.status(500).json({
                success: false,
                message: 'Unable to assign user role. Please contact administrator.'
            });
        }
        ...
        const newUser = new User({
            ...
            phone: phone || '',
            address: address || '',
            role: memberRole._id // Assign the member role ID
        });
