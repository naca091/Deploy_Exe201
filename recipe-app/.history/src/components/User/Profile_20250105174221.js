...
const Profile = () => {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                ...
                const response = await axios.get(`/api/users/${storedUser._id}`);
                setUser(response.data);
                form.setFieldsValue(response.data); // Đặt giá trị form từ dữ liệu API
            } catch (error) {
    ...
    const handleSaveProfile = async (values) => {
        try {
            ...
            setUser(response.data); // Cập nhật state với dữ liệu mới
    ...
    const handleChangePassword = async (values) => {
        try {
            await axios.post(`/api/users/${user._id}/change-password`, {
