...
const SeeVideo = () => {
    ...
    const checkRewardStatus = async (videoId) => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get(`https://demcalo.onrender.com/api/videos/check-reward/${videoId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
    ...
    const awardXu = async (videoId) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await axios.post('https://demcalo.onrender.com/api/videos/award-xu',
                { videoId },
                {
                    headers: {
                        'Authorization': `Bearer ${token}`
